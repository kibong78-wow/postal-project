...

{
  ...
  const styles = {
    ...
    logo: { fontSize: 18, fontWeight: 800, display: "flex", alignItems: "center", gap: 10 },
          ...
          <div>
            ...
            <div style={{ ...styles.card, borderLeft: `6px solid ${selectedLaw?.color || "#1e3a5f"}` }}>
              ...
              <div style={{ background: "#f8fafc", borderRadius: 10, padding: "20px 24px", marginBottom: 20 }}>
                <p style={{ fontSize: 15, lineHeight: 2, color: "#1e293b", margin: 0, whiteSpace: "pre-line" }}>
  ...
  const renderUpdate = () => {
    ...
    const addChangeRow = () => setUpdateChanges(p => [...p, { type:"added", target:"law", label:"", detail:"" }]);
    const removeRow = i => setUpdateChanges(p => p.filter((_,idx) => idx !== i));
    const updateRow = (i, field, val) => setUpdateChanges(p => p.map((r,idx) => idx===i ? {...r,[field]:val} : r));
    ...
    const handleSubmit = () => {
      ...
      const entry = {
        ...
        author: "관리자",
        summary: updateNote,
        changes: updateChanges.filter(c => c.label.trim()),
      };
      setVersionHistory(p => [entry, ...p]);
      setUpdateNote("");
      setUpdateChanges([{ type:"added", target:"law", label:"", detail:"" }]);
    ...
    return (
      <div>
        ...
        <div style={{ ...styles.card, borderTop: "4px solid #1e3a5f", marginBottom: 24 }}>
          <h2 style={{ ...styles.h2, marginBottom: 4 }}>🔄 DB 업데이트 요청</h2>
          <p style={{ fontSize: 13, color: "#64748b", marginBottom: 20, lineHeight: 1.7 }}>
            변경·추가·삭제할 내용을 아래에 작성하면 이 시스템의 대화창에 붙여넣어 업데이트를 요청할 수 있습니다.<br/>
            각 변경사항을 행별로 입력하고 <strong>요청 내용 생성</strong>을 누르세요.
          </p>

          {/* 변경 요약 */}
          <div style={{ marginBottom: 16 }}>
            <label style={{ fontSize: 13, fontWeight: 700, color: "#374151", display: "block", marginBottom: 6 }}>
              이번 업데이트 요약 <span style={{ color: "#dc2626" }}>*</span>
            </label>
            <input value={updateNote} onChange={e => setUpdateNote(e.target.value)}
              placeholder="예: 공무원 징계령 제17조 시효 규정 개정 반영 (2025.03)"
              style={{ ...styles.input }} />
          </div>

          {/* 변경 항목 행 */}
          <div style={{ marginBottom: 12 }}>
            <label style={{ fontSize: 13, fontWeight: 700, color: "#374151", display: "block", marginBottom: 8 }}>변경 항목</label>
            {updateChanges.map((row, i) => (
              <div key={i} style={{ display:"flex", gap:8, marginBottom:8, alignItems:"center", flexWrap:"wrap" }}>
                <select value={row.type} onChange={e => updateRow(i,"type",e.target.value)}
                  style={{ padding:"8px 10px", border:"2px solid #e2e8f0", borderRadius:8, fontSize:13, outline:"none", background:"#fff" }}>
                  <option value="added">추가</option>
                  <option value="modified">수정</option>
                  <option value="deleted">삭제</option>
                </select>
                <select value={row.target} onChange={e => updateRow(i,"target",e.target.value)}
                  style={{ padding:"8px 10px", border:"2px solid #e2e8f0", borderRadius:8, fontSize:13, outline:"none", background:"#fff" }}>
                  <option value="law">법령·조문</option>
                  <option value="card">업무카드</option>
                ...
                <input value={row.label} onChange={e => updateRow(i,"label",e.target.value)}
                  placeholder="항목명 (예: 근로기준법 제54조)"
                  style={{ ...styles.input, flex:1, minWidth:160, padding:"8px 12px" }} />
                <input value={row.detail} onChange={e => updateRow(i,"detail",e.target.value)}
                  placeholder="변경 상세 (예: 휴게시간 기준 4→3시간으로 개정)"
                  style={{ ...styles.input, flex:2, minWidth:200, padding:"8px 12px" }} />
                {updateChanges.length > 1 &&
                  <button onClick={() => removeRow(i)}
                    style={{ padding:"8px 12px", background:"#fee2e2", border:"none", borderRadius:8, color:"#dc2626", cursor:"pointer", fontWeight:700 }}>✕</button>
                }
              </div>
            ))}
            <button onClick={addChangeRow}
              style={{ ...styles.btn(false), fontSize:13, marginTop:4 }}>+ 항목 추가</button>
          </div>
          ...
          {updateNote.trim() && (
            <div style={{ marginBottom: 16 }}>
              ...
              <div style={{ background:"#f8fafc", border:"1px solid #e2e8f0", borderRadius:10, padding:16, fontSize:13, color:"#1e293b", lineHeight:1.8, position:"relative" }}>
                <pre style={{ margin:0, whiteSpace:"pre-wrap", fontFamily:"inherit" }}>
{`[DB 업데이트 요청]
요약: ${updateNote}

변경 항목:
${updateChanges.filter(c=>c.label.trim()).map(c =>
  `- [${typeLabel[c.type]}] (${targetLabel[c.target]}) ${c.label}${c.detail ? `: ${c.detail}` : ""}`
).join("\n")}

위 내용으로 postal-knowledge-base.jsx 파일의 DB를 업데이트하고 변경 이력에 추가해주세요.`}
                </pre>
                <button onClick={() => {
                  const text = `[DB 업데이트 요청]\n요약: ${updateNote}\n\n변경 항목:\n${updateChanges.filter(c=>c.label.trim()).map(c=>`- [${typeLabel[c.type]}] (${targetLabel[c.target]}) ${c.label}${c.detail?`: ${c.detail}`:""}`).join("\n")}\n\n위 내용으로 postal-knowledge-base.jsx 파일의 DB를 업데이트하고 변경 이력에 추가해주세요.`;
                  navigator.clipboard?.writeText(text);
                  handleSubmit();
                }} style={{ ...styles.btn(true), position:"absolute", top:12, right:12, fontSize:12, padding:"6px 14px" }}>
                  📋 복사 후 이력 등록
                </button>
              </div>
            </div>
          ...
          {updateSubmitted && (
            <div style={{ padding:"10px 16px", background:"#dcfce7", borderRadius:8, color:"#166534", fontWeight:700, fontSize:14, marginBottom:8 }}>
              ✅ 변경 이력이 등록되었습니다.
            </div>
          )}
        </div>
          ...
          <div key={vi} style={{ ...styles.card, borderLeft:"4px solid #1e3a5f", marginBottom:14 }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:12 }}>
              <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                ...
                <span style={{ fontSize:15, fontWeight:700, color:"#1e293b" }}>{v.summary}</span>
              </div>
            ...
            <div style={{ display:"flex", gap:8, marginBottom:12, flexWrap:"wrap" }}>
              {["added","modified","deleted"].map(t => {
                ...
                return cnt > 0 ? (
                  <span key={t} style={{ padding:"3px 10px", background:typeBg[t], color:typeColor[t],
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
              {v.changes.map((c,ci) => (
                <div key={ci} style={{ display:"flex", gap:8, alignItems:"flex-start", padding:"6px 10px",
                  background:"#f8fafc", borderRadius:8 }}>
                  <span style={{ padding:"2px 8px", background:typeBg[c.type], color:typeColor[c.type],
                    borderRadius:4, fontSize:11, fontWeight:700, whiteSpace:"nowrap" }}>
                    {typeLabel[c.type]}
                  </span>
                  <span style={{ padding:"2px 8px", background:"#eff6ff", color:"#1d4ed8",
    ...
    <div style={styles.app}>
      ...
      <header style={styles.header}>
        <div style={styles.logo}>
          <span>📮</span>
          <span style={{ fontSize: 16 }}>우정사업본부 업무 지식베이스</span>
        </div>
            ...
            <Section title="⚖️ 관련 법령">
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                {selectedCard.legal.map(lid => {
                  const l = LEGAL_BASES.find(x => x.id === lid);
                  return l ? (
                    <a key={lid} href={l.url} target="_blank" rel="noopener noreferrer"
